...
const settings = {
  ...
  subheadline: 'Medical courier and commercial logistics solutions built around reliable service, secure handling, and responsive regional coverage.',
  mainCTA: {
    ...
    <div className="space-y-10 md:space-y-16 lg:space-y-20 z-50 relative">
      ...
      <section className="relative flex flex-col gap-5 lg:gap-7 items-center text-center overflow-hidden pt-6 pb-10 md:pb-16">
        ...
        <Image
          ...
          height={820}
          aria-hidden
          className="pointer-events-none select-none absolute -top-24 right-1/2 translate-x-[55%] md:translate-x-[70%] w-[420px] md:w-[560px] h-auto opacity-[0.05] -z-10"
        />
          ...
          <div className="flex flex-col md:flex-row flex-wrap gap-3 md:gap-6 justify-center items-center">
            {settings.trustPoints.map((point) => (
              <div key={point.label} className="flex items-center gap-2 text-xs md:text-sm text-foreground">
                <point.icon size={16} className="text-teal shrink-0" />
